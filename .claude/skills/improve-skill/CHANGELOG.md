...
- [2026-08-11] Autonomous-push policy: the anneal commit now pushes when the build home's remote is yours; gated changes get rollback/review bracket tags (an ordinary anneal still gets none); background-run escalations push so someone can actually see them; static-skill proposals redirect to the private hub's queue in a publishing repo instead of going unsaved.
